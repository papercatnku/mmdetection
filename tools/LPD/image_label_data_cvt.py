...
class mulpti_process_base:
    ...
    def process_dir(self,src_dir,*args,**kwargs):
        ...
        pp.close()
        pp.join()
        self.mem_ls =self.mem_ls + list(filter(lambda x:not(x is None),total_out_ls))
...
class obj_det_cvt_base(mulpti_process_base):
            ...
            if lp_type:
                if ins_dict.get('polygon', False):
                    if not ins_dict['polygon'].get('point3', False):
                        ...
                        quad = pt_tolist(ins_dict['polygon']['point0']) + pt_tolist(ins_dict['polygon']['point1']) + pt_tolist(ins_dict['polygon']['point2']) + pt_tolist(ins_dict['polygon']['point3'])
                elif ins_dict.get('rect', False):
                    quad = bbox_to_quad(pt_tolist(ins_dict['rect']))
                else:
                    raise( AssertionError(f'no valid quad annotation in {label_fn}'))
    ...
    def export_coco(self, out_dir):
        ...
        coco_train_dict,coco_val_dict  = {"categories": cats}, {"categories": cats}

        next_image_id = 0
        next_anno_id = 0

        train_img_info_ls,  train_anno_info_ls, next_image_id, next_anno_id = self.make_coco_from_ls(self.train_ls,'-train',next_image_id, next_anno_id)
        val_img_info_ls,  val_anno_info_ls, next_image_id, next_anno_id = self.make_coco_from_ls(self.val_ls,'-train',next_image_id, next_anno_id)

        print(f'total image num: {next_image_id}\t annotation num:{next_anno_id}')

        print(f'train image num: {len(train_img_info_ls)}\t annotation num:{len(train_anno_info_ls)}')
        print(f'val image num: {len(val_img_info_ls)}\t annotation num:{len(val_anno_info_ls)}')
    ...
    def make_coco_from_ls(self, pair_ls,description_tag='',next_image_id=0,next_anno_id=0):
        ...
        for pair in track(pair_ls,description=f'makecoco{description_tag}',total=len(pair_ls)):
            ...
            filted_img_info, filted_label_infp = self.filt_quad_label(img_info, label_info)
            if filted_img_info:
                coco_img_inf = {
                    'id':next_image_id,
